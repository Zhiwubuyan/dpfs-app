...
export default function App() {
  ...
  const [currentSystemPage, setCurrentSystemPage] = useState(0); // 以 0 为起始下标
  ...
  const handleFetchSystemData = async (beginIndex = 0) => {
    ...
    try {
      ...
      console.error("Fetch System Data Error:", error);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <aside className="w-24 flex flex-col items-center py-10 bg-white border-r border-slate-100 z-50">
        ...
        <div className="flex-1 flex flex-col gap-10">
          <button
            ...
            className={`p-4 rounded-2xl transition-all ${activeTab === 'dashboard' ? 'bg-emerald-50 text-emerald-600 shadow-sm' : 'text-slate-300 hover:text-slate-600'}`}>
            <LayoutDashboard size={26} />
          </button>

          {/* 第二个图标：Activity - 系统数据查询 */}
          <button
            onClick={() => setActiveTab('activity')}
            className={`p-4 rounded-2xl transition-all ${activeTab === 'activity' ? 'bg-emerald-50 text-emerald-600 shadow-sm' : 'text-slate-300 hover:text-slate-600'}`}>
            <Activity size={26} />
          </button>

          <button
            onClick={() => setActiveTab('history')}
            className={`p-4 rounded-2xl transition-all ${activeTab === 'history' ? 'bg-emerald-50 text-emerald-600 shadow-sm' : 'text-slate-300 hover:text-slate-600'}`}>
            <Database size={26} />
          </button>
        </div>
              ...
              <div className="animate-in fade-in slide-in-from-left-4 duration-700">
                <header className="mb-12 flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="px-8 py-4 bg-emerald-600 text-white rounded-2xl font-bold flex items-center gap-2 hover:bg-emerald-500 shadow-xl shadow-emerald-950/10 transition-all"
                  >
                ...
                <div className="bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.02)] border border-slate-50 overflow-hidden mb-8">
                  ...
                  {systemData.length === 0 && !isLoading && (
                    <div className="py-20 text-center text-slate-300 italic font-medium">点击上方按钮同步系统数据</div>
                  )}
                </div>
                ...
                {systemTotal > 0 && (
                  <div className="flex justify-center items-center gap-6">
                    <button
                      ...
                      className="p-4 bg-white rounded-xl border border-slate-100 disabled:opacity-30 hover:text-emerald-500 transition-all"
                    >
                      <ChevronLeft size={20} />
                    </button>
                    <div className="text-sm font-bold text-slate-500">
                      第 {Math.floor(currentSystemPage / 20) + 1} / {Math.ceil(systemTotal / 20)} 页
                      <span className="ml-3 text-slate-300 font-normal">(总计 {systemTotal} 条)</span>
                    </div>
                    <button
              ...
              <div className="flex-1 bg-slate-950/40 rounded-[3.5rem] border border-white/5 backdrop-blur-3xl p-10 flex flex-col overflow-hidden">
                {isLoading ? (
                  ...
                  <div className="flex-1 flex flex-col animate-in fade-in slide-in-from-right-6 duration-1000">
                    ...
                    <div className="flex-1 bg-slate-900/80 rounded-3xl p-8 font-mono text-sm text-emerald-400/90 border border-white/5 mb-8 overflow-y-auto whitespace-pre-wrap">{riskReport}</div>
...
// 需要导入 RefreshCw, ChevronLeft 图标（已在头部添加）