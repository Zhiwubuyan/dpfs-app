    ...
    <div className="flex h-screen bg-[#F8FAFC] font-sans overflow-hidden text-slate-900 relative">
      ...
      <aside className="w-24 flex flex-col items-center py-10 bg-white border-r border-slate-100 z-50">
        ...
        <div className="flex-1 flex flex-col gap-10">
          ...
          {/* 关键修改点：
            将原来的“历史记录”（Database图标）按钮移到了第三位。
            将原来的“活动”（Activity图标）按钮移到了第二位。
          */}

          {/* 原第三个图标，现移至第二位：Activity */}
          <button className="p-4 text-slate-300 hover:text-slate-600 transition-colors">
            <Activity size={26} />
          </button>

          {/* 原第二个图标，现移至第三位：历史记录 (Database) */}
          <button
              ...
              <div className="animate-in fade-in slide-in-from-left-4 duration-700">
                <header className="mb-12 flex justify-between items-end">
                  ...
                  <div className="flex gap-4">
                    <div className="relative">
                      ...
                      <input type="text" placeholder="搜索历史批次..." className="pl-12 pr-6 py-3 bg-white border border-slate-100 rounded-2xl text-sm focus:ring-4 focus:ring-emerald-500/5 transition-all outline-none w-64 shadow-sm" />
                    </div>
                  </div>
                </header>
                ...
                <div className="bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.02)] border border-slate-50 overflow-hidden">
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {historyItems.map((item) => (
                        <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                          ...
                          <td className="px-8 py-6">
                            <span className="px-3 py-1 bg-slate-100 rounded-lg text-[10px] font-bold text-slate-500 uppercase">{item.mode}</span>
                          </td>
                          <td className="px-8 py-6 font-mono text-slate-600 font-bold">{item.qty} units</td>
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-2 text-emerald-600 font-black text-[10px] tracking-widest">
                  ...
                  {historyItems.length === 0 && (
                    <div className="py-20 text-center text-slate-300 italic font-medium">暂无历史评估记录</div>
                  )}
                </div>
              </div>
            )}
